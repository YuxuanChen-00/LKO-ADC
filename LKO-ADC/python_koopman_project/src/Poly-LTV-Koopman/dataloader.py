...
def generate_koopman_data(control, states, time_step, pred_step=1):
    ...
    此版本以逆序方式构建时间延迟向量，以匹配特定的MATLAB实现。
    """
    # 确保输入是numpy数组
    ...
    for i in range(num_samples):
        # --- 主要修改点 ---
        # 定义输入状态历史的索引窗口，从正序改为逆序。
        # 这种方式会生成如 [k, k-1, ..., k-time_step+1] 的索引。
        # 原代码: history_indices = np.arange(i, i + time_step, 1)
        ...
        for k_idx in range(pred_step):
            ...
            # 标签是状态窗口在未来偏移k步的结果
            # 由于history_indices是逆序的，label_indices和control_indices也会自然保持一致的逆序结构
