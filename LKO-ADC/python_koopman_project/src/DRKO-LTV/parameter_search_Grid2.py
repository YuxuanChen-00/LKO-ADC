...
def main():
    ...
    trainable_with_resources = tune.with_resources(train_model, resources={"cpu": 1, "gpu": 0.2})
    ...
    # 这是比检查 'error' 列更稳妥的方法
    print(f"原始试验总数: {len(results_df)}")
    results_df_successful = results_df.dropna(subset=['rmse']).copy()
    results_df_successful = results_df_successful[np.isfinite(results_df_successful['rmse'])]
    print(f"有效的成功试验数: {len(results_df_successful)}")
    ...
    if results_df_successful.empty:
        ...
        print(f"所有试验的原始结果（可能包含错误或无效指标）已保存至: {full_results_csv_path}")
        ray.shutdown()
        return
    ...
    full_results_csv_path = BASE_MODEL_SAVE_PATH / "grid_search_full_raw_results.csv"
    results_df.to_csv(full_results_csv_path, index=False)
    print(f"所有试验的原始结果（含无效指标）已保存至: {full_results_csv_path}")
    ...
    try:
        ...
        best_delay_step = int(best_params_combo['delay_step'])
        best_i_multiplier = int(best_params_combo['i_multiplier'])

        # 从所有成功的试验中，筛选出具有最优参数组合的那些
        ...
        # Ray Tune可能会创建多个checkpoint目录，找到最新的一个
        checkpoint_dirs = list(best_trial_path.glob("checkpoint_*"))

        if checkpoint_dirs:
            best_checkpoint_dir = max(checkpoint_dirs, key=lambda p: p.stat().st_mtime)
            ...
            if source_model_path.exists():
                ...
                print(f"来源试验的最佳Seed: {best_trial_row.get('seed', 'N/A')}")
