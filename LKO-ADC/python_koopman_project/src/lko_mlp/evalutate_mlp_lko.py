...
# from data_utils import generate_mlp_data
...
# --- 核心评估函数 ---
def evaluate_mlp_lko(model, control_data, initial_state, true_labels, delay_step):
    """
    在闭环模式下评估mlp模型，模拟MATLAB的lko（leave-k-out）评估逻辑。
    ...
    predict_step = control_data.shape[0]
    state_size = initial_state.shape[2]
    ...
    with torch.no_grad():  # 在此上下文中不计算梯度
        for i in range(predict_step):
            # 准备当前步的控制输入，形状变为 (1, c) 以匹配批处理输入

            current_control = control_data[i, :].unsqueeze(0)

            # PyTorch模型的前向传播，假定模型接收 state 和 control 两个输入
            # state shape: (1, seq_len, d), control shape: (1, c)
            phi_current, phi_pred = model(current_state, current_control)

            # 假设模型输出 phi_pred 的形状是 (1, output_features)
            # 移除批处理维度，方便处理
            phi_pred_flat = phi_pred.squeeze(0)

            # 提取并存储当前步的预测值 (预测结果是输出的前 state_size 个元素)
            step_prediction = phi_pred_flat[:state_size]
            y_pred_list.append(step_prediction)
    ...
    # 3. 后处理
    # 将预测列表堆叠成一个张量 (d, predict_step)

    y_pred = torch.stack(y_pred_list, dim=1)

    y_true = true_labels[:, 0:predict_step]
...
if __name__ == '__main__':
    # 假设我们已经有了上一部分定义的 generate_mlp_data 函数
    from __main__ import generate_mlp_data
    ...
    ctrl_gen, state_gen, label_gen = generate_mlp_data(
    ...
    # 4. 运行评估函数
    rmse_val, y_true_out, y_pred_out = evaluate_mlp_lko(
        model=model,
