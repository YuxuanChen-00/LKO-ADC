...
eta_A = 4e-12;  % A_bar 的学习率 
eta_B = 1e-3;   % B_bar 的学习率 

% ==================== 新增：混合更新律的权重参数 ====================
lambda = 0;   % 轨迹跟踪误差项的权重 (0 <= lambda <= 1)
                % lambda = 0: 只关心模型准确度 (与您之前的代码相同)
                % lambda = 1: 只关心轨迹跟踪准确度
% =================================================================
...
for k = 1:k_steps
    ...
    if k >= window_size
        % === 第一部分：计算基于模型预测误差的梯度 ===
        [grad_A_model, grad_B_model] = update_AB_sliding_window(A_bar, B_bar, ...
                                              X_win_hist, U_win_hist, X_next_win_hist);
        
        % === 第二部分：计算基于轨迹跟踪误差的梯度 ===
        % 计算当前状态的轨迹跟踪误差 e = y_k - y_ref_k
        % 注意: Y_history(:,k) 存储的是 y_{k-1} 的值, Y_history(:,k+1) 存储的是 y_k 的值
        current_Y_output = Y_history(:,k+1); 
        current_Y_ref = Y_ref(:, k); % 获取当前步的参考点
        tracking_error = current_Y_output - current_Y_ref;
        
        % 计算跟踪误差项的梯度
        % 梯度形式为 -C' * e_track * regressor'
        % 这里我们使用当前状态 X_koopman_current (x_k) 和 当前输入 current_U (u_k)作为回归量
        grad_A_track = -C' * tracking_error * X_koopman_current';
        grad_B_track = -C' * tracking_error * current_U';
        
        % === 第三部分：组合梯度并更新模型 ===
        % 最终梯度是两部分梯度的加权和
        grad_A_total = (1 - lambda) * grad_A_model + lambda * grad_A_track;
        grad_B_total = (1 - lambda) * grad_B_model + lambda * grad_B_track;
        
        % 应用梯度下降更新模型
        A_bar = A_bar - eta_A * grad_A_total;
        B_bar = B_bar - eta_B * grad_B_total;
        
        % 增加调试信息，监控更新幅度
        if mod(k, 20) == 0 % 每20步打印一次
            update_norm_A = norm(eta_A * grad_A_total, 'fro');
            update_norm_B = norm(eta_B * grad_B_total, 'fro');
...
view(3);
figure;
plot3(Y_history(4,:), Y_history(5,:), Y_history(6,:), 'c-', 'LineWidth', 1.5);
hold on;
plot3(Y_ref(4,1:k_steps+1), Y_ref(5,1:k_steps+1), Y_ref(6,1:k_steps+1), 'm--', 'LineWidth', 1.5);
...
ylabel('Y position');
zlabel('Z position');
title('3D Trajectory Tracking - Joint Group 2');
axis equal;
grid on;
legend('Actual Trajectory', 'Reference Trajectory', 'Actual Start', 'Reference Start');
view(3);
...
%   grad_A_avg, grad_B_avg: 计算出的A和B的平均梯度
    ...
    for i = 1:N_window
        ...
        x_i = X_window(:, i);
        u_i = U_window(:, i);
        x_i_plus_1_real = X_next_window(:, i); 
        % 使用当前模型进行预测
        x_i_plus_1_pred = A_current * x_i + B_current * u_i;
        
        % 计算状态预测误差 (模型误差)
        model_error_vec = x_i_plus_1_pred - x_i_plus_1_real;
        
        % 计算损失函数对A和B的梯度: L = 1/2 * ||error||^2
        % dL/dA = dL/de * de/dA = error * x_i'
        grad_A_single = model_error_vec * x_i';
        
        % dL/dB = dL/de * de/dB = error * u_i'
        grad_B_single = model_error_vec * u_i';
