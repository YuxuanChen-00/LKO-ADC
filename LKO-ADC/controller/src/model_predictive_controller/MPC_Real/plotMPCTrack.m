...
predict_steps = size(Y_history, 2); 

%% 计算均方根误差
mse1 = calculateRMSE(Y_ref(1:3, 1:predict_steps), Y_history(1:3,:));
mse2 = calculateRMSE(Y_ref(4:6, 1:predict_steps), Y_history(4:6,:));
fprintf('第一关节轨迹跟踪均方根误差为: %2f, 第二关节轨迹跟踪的均方根误差为: %2f\n', mse1, mse2);


%% 绘制结果
time_vec = 1:predict_steps; % 时间向量，对应 Y_history 和 X_koopman_history
time_vec_input = 1:predict_steps-1;

% 1. 3D轨迹跟踪效果 (假设Y的前三维是x,y,z坐标)
figure;
plot3(Y_history(1,:), Y_history(2,:), Y_history(3,:), 'b-', 'LineWidth', 1.5);
hold on;
plot3(Y_ref(1,1:predict_steps), Y_ref(2,1:predict_steps), Y_ref(3,1:predict_steps), 'r--', 'LineWidth', 1.5);
...
hold on;
plot3(Y_ref(4,1:predict_steps), Y_ref(5,1:predict_steps), Y_ref(6,1:predict_steps), 'r--', 'LineWidth', 1.5);
