function trajectory = generateReferenceCircle(initial_pos, radius, alpha, steps)
...
% 计算姿态参数（末端z轴的方向余弦）
nx = sin(theta) * sin(alpha);
ny = -cos(theta) * sin(alpha);
nz = cos(alpha) * ones(size(theta));

% 合并轨迹数据
trajectory = [x; y; z; nx; ny; nz];
