...
diary(fullfile(mainFolder, 'parameter_search_log.txt'));
diary on;
...
for combo_idx = 1:total_combos
    ...
    try
        ...
        % 计算Koopman算子（带正则化）
        [A, B] = compute_koopman(control_td, state_phi, label_phi, lambda);
        ...
        if mod(combo_idx,5) == 0
            save(fullfile(mainFolder, 'temp_results.mat'), 'results', '-v7.3');
        end
...
writetable(report_table, fullfile(mainFolder, 'performance_report.csv'));

% 保存完整结果
save(fullfile(mainFolder, 'full_results.mat'), 'sorted_results', '-v7.3');
