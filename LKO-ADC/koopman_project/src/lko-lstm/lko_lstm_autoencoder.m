% 文件名：CustomNetwork.m
classdef lko_lstm_autoencoder
    properties
        Net             % 存储dlnetwork对象
    end
    
    methods
        function obj = lko_lstm_autoencoder(state_size, hidden_size, output_size, control_size, time_step)
            % 构造函数：构建网络结构并初始化
            ...
            % 连接层
            obj.Net = connectLayers(obj.Net, 'tanh2', 'reshape');
            obj.Net = connectLayers(obj.Net, 'reshape1', 'A');
            ...
            recoveryNet = [
                ...
                tanhLayer()
                fullyConnectedLayer(state_size, 'Name', 'fc_phi3')
            ];
