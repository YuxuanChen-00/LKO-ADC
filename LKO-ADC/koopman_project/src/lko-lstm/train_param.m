...
param_combinations.delay_time = 6;     % 迭代的延迟步长值
param_combinations.phi_dimensions = 7:30; % 迭代的Phi维度值
param_combinations.hidden_size = int16(param_combinations.phi_dimensions*3/4); 

% 生成所有参数组合
delay_list = param_combinations.delay_time;
phi_list = param_combinations.phi_dimensions;
[delay_grid, phi_grid] = ndgrid(delay_list, phi_list);
param_sets = [delay_grid(:), phi_grid(:)];

%% 固定参数配置
base_params = struct();
base_params.state_size = 6;            % 固定：特征维度
base_params.control_size = 6;          % 固定：控制输入维度
base_params.initialLearnRate = 0.001;   % 固定：初始学习率
base_params.minLearnRate = 0;          % 固定：最低学习率
base_params.num_epochs = 1000;         % 固定：训练轮数
base_params.L1 = 100;                    % 固定：损失权重1
...
for i_set = 1:size(param_sets, 1)
    ...
    curr_phi = param_sets(i_set, 2);
    
    % 创建参数结构体
    params = base_params;
    params.delay_step = curr_delay;
    params.hidden_size = int32((curr_phi+params.state_size)/2);
    ...
    % 保存模型和结果
    save(fullfile(model_save_path, 'trained_network.mat'), 'net');
